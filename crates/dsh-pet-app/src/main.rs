...
use dsh_pet_core::{Config, Mode, PetState, RpcClient, Snapshot, SseConnector, StateEvent};
use dsh_pet_ui::{create_window, drag_delta, InputAction, InputState, Renderer};
...
use crate::tasks::{poll_task, sse_host_task, sse_mux_task, tick_task, DSH_URL_DEFAULT};
...
fn main() -> anyhow::Result<()> {
    ...
    // DSH URL
    let dsh_url = std::env::var("DSH_PET_URL")
        .unwrap_or_else(|_| DSH_URL_DEFAULT.to_string())
        .trim_end_matches('/')
        .to_string();
    ...
    let http = reqwest::Client::builder()
        ...
        .build()?;
    let rpc_client = Arc::new(RpcClient::new(dsh_url.clone(), http.clone()));
    let sse_mux = Arc::new(SseConnector::new(
        format!("{dsh_url}/api/events.mux"),
        http.clone(),
    ));
    let sse_host = Arc::new(SseConnector::new(
        format!("{dsh_url}/api/events.host"),
        http.clone(),
    ));
    ...
    rt.spawn(tick_task(event_tx.clone(), cancel.clone()));
    ...
    // 托盘
    let _tray = tray::create(config.sound_on, config.bubble_visible, config.scale);
        ...
        match event {
            Event::WindowEvent {
                ...
                WindowEvent::CursorMoved { position, .. } => {
                    ...
                    if let Some((dx, dy)) = drag_delta(&mut input, position) {
            ...
            Event::UserEvent(UserEvent::TrayAction(action)) => {
                handle_tray_action(action, &mut config, &mut renderer, &audio, &dsh_url);
                window.request_redraw();
            }
            Event::AboutToWait => {
