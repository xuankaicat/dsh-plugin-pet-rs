...
**由桌宠启动 DSH**：开启后桌宠会自动拉起 DSH Web 作为子进程并自动切换连接，无需手动填地址。启动方式自动按顺序尝试：`dsh`（PATH 中已安装）→ `cmd /C dsh`（Windows shim）；若环境中没有 dsh，会先弹出气泡确认是否需要安装，并可选安装源：**官方源**（`registry.npmjs.org`）、**npmmirror**（阿里，`registry.npmmirror.com`）或**腾讯源**（`mirrors.cloud.tencent.com/npm`）。确认后执行 `npm install -g @deepseek-ai/dsh --verbose --registry=<源>`，通过气泡实时显示 npm 下载进度，安装完成后自动重试启动；选择「取消」则不安装。安装目录可点「选择安装目录…」用系统文件夹选择器指定；未指定时 Windows 默认安装到 `D:\dsh`（D 盘存在时），否则用 npm 默认位置。均优先使用默认端口 `3080`，被占用时回退 `--port 0` 由系统分配空闲端口（后台静默启动，不弹 CLI 窗口）。此时「DSH 地址」为只读显示，右侧「重启」按钮可重启该子进程（重启后自动重连新地址）。关闭该开关时会弹出确认警告（Windows）：将终止当前由桌宠启动的 DSH 进程；确认后停止子进程并恢复手动地址输入。桌宠退出（含托盘退出）时会一并回收子进程。
